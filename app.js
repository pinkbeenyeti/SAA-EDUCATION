...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderQuestionPracticePanel(questions, practice, refs, emptyMessage, rerender) {
    ...
    if (isRevealed) {
      ...
      refs.lblExplanationText.innerHTML = renderExplanationStructured(isKo ? q.explanation_ko : q.explanation_en, isKo);
    } else {
      refs.explanationBox.style.display = 'none';
    }
  ...
  function renderCurrentQuestion() {
    ...
    if (isRevealed) {
      ...
      el.lblExplanationText.innerHTML = renderExplanationStructured(isKo ? q.explanation_ko : q.explanation_en, isKo);
    } else {
    ...
    state.incorrectNotes.forEach((item, idx) => {
      ...
      card.innerHTML = `
        ...
        <div class="explanation-box" style="margin-top: 0;">
          ...
          <div class="explanation-content">${renderExplanationStructured(isKo ? q.explanation_ko : q.explanation_en, isKo)}</div>
        </div>
      `;
  ...
  // extra markup.
  // Structured explanation: { summary, options: [{correct, reason}, x4] },
  // aligned by array INDEX to the question's current options_ko/en order --
  // never by letter. The letter shown per row is derived here, at render
  // time, from that index, so it can never go stale the way a letter cited
  // inside freeform prose could after generate_questions.py's
  // reshuffle_options() reorders the options post-hoc. `correct` always
  // comes from data.js's answer field via merge_generated.py, never from
  // free-text parsing, so the highlighting is guaranteed consistent with the
  // actual graded answer.
  // Falls back to the legacy freeform-string path for any explanation that
  // hasn't been migrated to the structured schema yet (e.g. LOCAL_DUMP_BANK,
  // which is regenerated by a separate local-only script).
  function renderExplanationStructured(explanation, isKo) {
    if (!explanation) return '';
    if (typeof explanation === 'string') return renderExplanationMarkdown(explanation);

    const parts = [];
    if (explanation.summary) {
      parts.push(`<div class="exp-summary">${renderExplanationMarkdown(explanation.summary)}</div>`);
    }
    if (Array.isArray(explanation.options)) {
      const rows = explanation.options.map((opt, i) => {
        const letter = String.fromCharCode(65 + i);
        const cls = opt.correct ? 'exp-opt-correct' : 'exp-opt-wrong';
        const icon = opt.correct ? '✅' : '❌';
        return `
          <div class="exp-option-row ${cls}">
            <div class="exp-option-marker">${icon} ${letter}</div>
            <div class="exp-option-reason">${renderExplanationMarkdown(opt.reason || '')}</div>
          </div>`;
      }).join('');
      parts.push(`<div class="exp-options-list">${rows}</div>`);
    }
    return parts.join('');
  }
