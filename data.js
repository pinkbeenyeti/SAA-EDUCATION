...
/**
 * Dump Importer Helper Functions
 */
function validateQuestionFormat(item) {
  if (!item || typeof item !== 'object') return false;
  const hasKo = typeof item.question_ko === 'string' && Array.isArray(item.options_ko) && item.options_ko.length === 4;
  const hasEn = typeof item.question_en === 'string' && Array.isArray(item.options_en) && item.options_en.length === 4;
  const hasAnswer = typeof item.answer === 'number' && item.answer >= 0 && item.answer < 4;
  return (hasKo || hasEn) && hasAnswer;
}

function importDumpQuestions(jsonOrArray) {
  let questions = [];
  try {
    if (typeof jsonOrArray === 'string') {
      questions = JSON.parse(jsonOrArray);
    } else if (Array.isArray(jsonOrArray)) {
      questions = jsonOrArray;
    }
  } catch (err) {
    return { success: false, message: 'Invalid JSON format: ' + err.message, count: 0 };
  }

  if (!Array.isArray(questions)) {
    return { success: false, message: 'Input data must be an array of question objects.', count: 0 };
  }

  const validQuestions = [];
  questions.forEach((q, idx) => {
    // Fill in defaults if partially formatted
    const normalized = {
      id: q.id || 'dump_' + Date.now() + '_' + idx,
      service_id: q.service_id || 'general',
      domain_id: q.domain_id || 'compute',
      difficulty: q.difficulty || 'Medium',
      question_ko: q.question_ko || q.question || q.question_en || '문제 번역 준비 중',
      question_en: q.question_en || q.question || q.question_ko || 'Question in preparation',
      options_ko: Array.isArray(q.options_ko) ? q.options_ko : (Array.isArray(q.options) ? q.options : ['A', 'B', 'C', 'D']),
      options_en: Array.isArray(q.options_en) ? q.options_en : (Array.isArray(q.options) ? q.options : ['A', 'B', 'C', 'D']),
      answer: typeof q.answer === 'number' ? q.answer : 0,
      explanation_ko: q.explanation_ko || q.explanation || '해설 준비 중',
      explanation_en: q.explanation_en || q.explanation || 'Explanation pending.'
    };

    if (validateQuestionFormat(normalized)) {
      validQuestions.push(normalized);
    }
  });

  if (validQuestions.length === 0) {
    return { success: false, message: 'No valid questions found in data.', count: 0 };
  }

  // Save to LocalStorage
  try {
    const existing = JSON.parse(localStorage.getItem('aws_saa_custom_dump') || '[]');
    const merged = [...existing, ...validQuestions];
    localStorage.setItem('aws_saa_custom_dump', JSON.stringify(merged));
    return { success: true, count: validQuestions.length, totalCustom: merged.length };
  } catch (e) {
    return { success: false, message: 'LocalStorage Error: ' + e.message, count: 0 };
  }
}
...
function getAllQuestions() {
  let custom = [];
  try {
    custom = JSON.parse(localStorage.getItem('aws_saa_custom_dump') || '[]');
  } catch (e) {
    console.error('Failed to load custom dump:', e);
  }
